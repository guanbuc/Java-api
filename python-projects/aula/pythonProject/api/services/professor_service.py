from ..models import professor_models
from api import db

def cadastrar_professor(professor):
    professor_bd = professor_models.Professor(nome=professor.nome, idade=professor.idade)
    db.session.add(professor_bd)
    db.session.commit()
    return professor_bd

def listar_professores():
    professores = professor_models.Professor.query.all()
    return professores

def listar_professor_id(id):
    professor  = professor_models.Professor.query.filter_by(id=id).first()
    return professor
