...
def cadastrar_curso(curso):
    curso_bd = curso_models.Curso(nome=curso.nome, descricao=curso.descricao,
...
def listar_cursos():
    cursos = curso_models.Curso.query.all()
    return cursos

def listar_curso_id(id):
    curso  = curso_models.Curso.query.filter_by(id=id).first()
    return curso
